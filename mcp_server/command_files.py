...
**Sequence enforcer hook (loaded from `.cursor/hooks/sequence_hook.py`):**

| Hook | Purpose |
|---|---|
| `sequence_enforcer` | Enforce prerequisite tool-call ordering per session (see below) |

## Sequence enforcer (`mcp_server/sequence_enforcer.py`)

Adds a `CallLedger` — a thread-safe, append-only log of every tool call this
session — and a ruleset of `SequenceRule` objects that gate certain tools
behind prerequisites:

```
tool must only run after [prerequisite list] has been called
```

**Built-in rules (`DEFAULT_SEQUENCES`):**

| Tool | Requires first | Reason |
|---|---|---|
| `graph_commit` | `system_status` | Confirm env health before vault mutation |
| `graph_ingest` | `graph_status` | Verify graph integrity before adding nodes |
| `graph_ingest_source` | `graph_status` | Same |
| `graph_link` | `graph_status` | Same |
| `harmonic_propagate` | `harmonic_index_state` | Read shard state before propagating |
| `harmonic_inject` | `harmonic_index_state` | Same |
| `hub_inject` | `hub_state` | Read hub before injecting |
| `harmonic_reset` | `harmonic_index_state` | Capture state before reset |
| `sweep_attractors` | `harmonic_index_state` | Simulation sanity |
| `ana_chi_sim` | `harmonic_index_state` | Simulation sanity |

**Notion-aware rule (added by `sequence_hook.py`):**

| Tool | Requires first | Reason |
|---|---|---|
| `graph_commit` | `sync_notion` | If Notion sync was run this session, commit after sync |

Disable: `SPOTIFY_RIP_DISABLE_SEQUENCE_ENFORCER=1`

Extend with extra rules:

```python
from mcp_server.sequence_enforcer import SequenceRule, build_enforcer

enforcer = build_enforcer(extra_rules=[
    SequenceRule(
        tool     = "my_mutating_tool",
        requires = ["my_inspect_tool"],
        message  = "Inspect before mutating.",
    ),
])
```

## Notion → Obsidian flow (`graph/notion_ingestion.py`)

The **reservoir** (Notion) and the **graph** (Obsidian vault) are separate
layers with a one-way sync pipeline:
...
Notion (reservoir)
  └─ databases, pages, blocks
        ...
        │ notion_to_md()
        ▼
  Obsidian vault/notion/*.md
        │ IngestionPipeline (embed + link)
        ▼
  harmonic index  ←  graph_commit
```

**Credentials:** `NOTION_API_TOKEN` (env var / Cursor secret)

**MCP tool:** `sync_notion(max_pages=0, fetch_timeout=30)`

**Recommended session flow:**

```
system_status()        → open gate, confirm env health
sync_notion()          → pull Notion reservoir → vault nodes
graph_status()         → verify graph integrity
graph_commit(...)      → commit session + Notion nodes to harmonic index
```

The sequence enforcer ensures this order is respected automatically.
