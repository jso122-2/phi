...
> created_ts: 2025-05-15-063837  
> Created: 2025-05-15 06:38 UTC  
> Edited: 2025-05-15 06:39 UTC  
> Source: Google Keep  

---

1	Either populate or drop the two empty files
contemporary_bugfix.jsonl.txt & reflective_prompts.jsonl	Remove zero-signal placeholders that break samplers	• If keeping: generate ≥ 500 rows via your bug-fix & reflective‐prompt generators.
• If dropping: git rm contemporary_bugfix.jsonl.txt reflective_prompts.jsonl

3	Token-length clipping for giant text sets (core_textbook, formatted_training, unified_reasoning)	Avoid >2048-token overflow in causal-LM prefix	python\nfrom transformers import AutoTokenizer\ntok = AutoTokenizer.from_pretrained(\"gpt2\")\nclip_jsonl(\"core_textbook.jsonl\", tok, 2048)\n

4	Near-duplicate pruning across MathWorld & textbooks	Reduce overweighting of identical passages	bash\npython dedupe_minhash.py --input math_technical.jsonl MW_10k*.jsonl --threshold 0.9 --output math_dedup.jsonl\n

5	Upsample tiny but critical sets (manhattan_lsat_games, Paper_Texts, WikiGlossaries)	≥1 000 lines each to balance training signal	• Run your LSAT spider (fetch_lsat.py) to pull more games.
• Extract additional glossary terms with the Scrapy spider template.
...
→ [[keep/2025-05-15-113345-pretty-code]]
